...
def main(config):
    ...
    for i in range(1,27):
        ...
        for epoch in range(1, max_epoch + 1):
            ...
            if epoch <= max_epoch:
                torch.save(save_obj, os.path.join('./save/pre/casme2-4', str(i) + 'epoch-last.pth'))
        ...
        for epoch in range(1, max_epoch + 1):
            ...
            for name, loader, name_l, name_a in [
                ...
                np.random.seed(0)
                for i, (input_cha, index) in enumerate(sup_loader):
                    for j, (input_cha1, index1) in enumerate(val_loader):
                        target = index.cuda()
